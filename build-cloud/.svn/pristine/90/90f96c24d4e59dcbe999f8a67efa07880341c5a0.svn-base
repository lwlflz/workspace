...
/**
 * @ClassName: JSONSerializeUtil
 * @Description: json序列化相关帮助类
 * @author: liutao
 * @date: 2018年4月2日 上午10:40:41
 */
public class JSONSerializeUtil {
	private static Map<Class<?>, ObjectWriter> objWriterCache = Maps.newConcurrentMap(); // 缓存class到ObjectWriter的
	// 映射关系,格式为Class - > ObjectWriter对象
	private static Map<Class<?>, ObjectWriter> objWriterCacheNoType = Maps.newConcurrentMap(); // 缓存class到
	// ObjectWriter的映射关系,格式为Class - > ObjectWriter对象
	private static Map<Class<?>, ObjectWriter> objWriterCacheNoNull = Maps.newConcurrentMap(); // 缓存class到
	// ObjectWriter的映射关系,格式为Class - > ObjectWriter对象
	...
	public static <T> T jsonReSerializer(String jsonStr, Class<T> calzz) {
		try {
			mapper.enableDefaultTyping(DefaultTyping.NON_FINAL);
			ObjectReader reader = mapper.readerFor(calzz);
	...
	public static <T> T jsonReSerializerNoType(String jsonStr, Class<T> calzz) {
		try {
			ObjectReader reader = mapperNoType.readerFor(calzz);
			T object = reader.readValue(jsonStr);
			return object;
		} catch (Exception e) {
			throw new RuntimeException(String.format("字符串[%.20s]...反序列化失败。", jsonStr), e);
	...
	public static <T> T jsonReSerializerNoNull(String jsonStr, Class<T> calzz) {
		try {
			ObjectReader reader = mapperNoNull.readerFor(calzz);
			T object = reader.readValue(jsonStr);
			return object;
		} catch (Exception e) {
			throw new RuntimeException(String.format("字符串[%.20s]...反序列化失败。", jsonStr), e);
	...
	public static <T> T jsonReSerializerNoType(String jsonStr, TypeReference<T> typeReference) {
		try {
			ObjectReader reader = mapperNoType.readerFor(typeReference);
			T object = reader.readValue(jsonStr);
			return object;
		} catch (Exception e) {
			throw new RuntimeException(String.format("字符串[%.20s]...反序列化失败。", jsonStr), e);
